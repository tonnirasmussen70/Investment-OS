...
old_table = '''        table = section[["Name", "Quantity", "Purchase_Price", "Position_Display_Price", "Sector", "Market_Value_DKK", "Portfolio_Weight_Ex_Grundfos", "Return_Pct", "Composite", "AI_Confidence"]].copy()'''
new_table = '''        display_section = section.loc[~grundfos_mask].copy() if asset_class == "Aktie" else section.copy()\n        table = display_section[["Name", "Quantity", "Purchase_Price", "Position_Display_Price", "Sector", "Market_Value_DKK", "Portfolio_Weight_Ex_Grundfos", "Return_Pct", "Composite", "AI_Confidence"]].copy()'''
if old_table not in text:
    raise RuntimeError("Kunne ikke finde Positioner-tabelkilden")
text = text.replace(old_table, new_table, 1)

APP.write_text(text, encoding="utf-8")
print("Positioner dagskurs = Yahoo, og Grundfos skjules før subtotal")
